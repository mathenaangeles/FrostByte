...
create schema if not exists FROSTBYTE_DB.APP;

use database FROSTBYTE_DB;
use schema APP;

create or replace table FROSTBYTE_CONFIG (
  CONFIG_NAME string,
  INPUT_TABLE string,
  INPUT_REF string,
  DATE_COL string,
...
create or replace table FROSTBYTE_RUNS (
  RUN_ID string,
  CONFIG_NAME string,
  STATUS string,
...
create or replace table FROSTBYTE_DQ_RUN (
  RUN_ID string,
  CONFIG_NAME string,
  COMPUTED_AT timestamp,
  START_DATE date,
...
create or replace table FROSTBYTE_DQ_SERIES (
  RUN_ID string,
  CONFIG_NAME string,
  COMPUTED_AT timestamp,
  ORG_UNIT string,
...
create or replace table FROSTBYTE_SERIES_WEEKLY (
  RUN_ID string,
  CONFIG_NAME string,
  COMPUTED_AT timestamp,
  WEEK_START date,
...
create or replace table FROSTBYTE_ANOMALIES_WEEKLY (
  RUN_ID string,
  CONFIG_NAME string,
  COMPUTED_AT timestamp,
  WEEK_START date,
...
create or replace table FROSTBYTE_FORECAST_4W (
  RUN_ID string,
  CONFIG_NAME string,
  COMPUTED_AT timestamp,
...
create or replace table FROSTBYTE_SERIES_FEATURES (
  RUN_ID string,
  CONFIG_NAME string,
  COMPUTED_AT timestamp,
  ORG_UNIT string,
...
create or replace table FROSTBYTE_SERIES_CLUSTERS (
  RUN_ID string,
  COMPUTED_AT timestamp,
  ORG_UNIT string,
  METRIC_NAME string,
...
create or replace table FROSTBYTE_ACTIONS (
  RUN_ID string,
  CONFIG_NAME string,
  COMPUTED_AT timestamp,
  ACTION_TYPE string,
...
create or replace table FROSTBYTE_WEEKLY_BRIEF (
  RUN_ID string,
  CONFIG_NAME string,
  COMPUTED_AT timestamp,
  WEEK_START date,
...
create or replace table FROSTBYTE_AI_AUDIT (
  RUN_ID string,
  CONFIG_NAME string,
  COMPUTED_AT timestamp,
  PURPOSE string,
...
create or replace table FROSTBYTE_DEMO_METRIC_DAILY (
  DT date,
  ORG_UNIT string,
  METRIC_NAME string,
  METRIC_VALUE float,
...
truncate table FROSTBYTE_DEMO_METRIC_DAILY;

insert into FROSTBYTE_DEMO_METRIC_DAILY
with d as (
...
merge into FROSTBYTE_CONFIG t
using (
  select
    'demo'::string as CONFIG_NAME,
    'FROSTBYTE_DB.APP.FROSTBYTE_DEMO_METRIC_DAILY'::string as INPUT_TABLE,
    null::string as INPUT_REF,
...
create or replace procedure RUN_FROSTBYTE_CLUSTER(RUN_ID string)
returns string
language python
...
def main(session: Session, run_id: str) -> str:
    ...
    session.sql("delete from FROSTBYTE_SERIES_CLUSTERS where RUN_ID = '{}'".format(rid)).collect()
    df = session.sql(
        "select ORG_UNIT, METRIC_NAME, CATEGORY, DIM1, DIM2, DIM3, LEVEL_MEAN, VOLATILITY, SLOPE, RECENT_WOW_ABS, ANOMALY_RATE, MISSING_PCT "
        "from FROSTBYTE_SERIES_FEATURES where RUN_ID = '{}'".format(rid)
    ).to_pandas()
    if df.empty:
        return "NO_FEATURES"
    ...
    session.write_pandas(out, "FROSTBYTE_SERIES_CLUSTERS", auto_create_table=False, overwrite=False)
    return "CLUSTERED:{}".format(k)
$$;

create or replace procedure RUN_FROSTBYTE(CONFIG string)
returns string
language sql
execute as owner
as
...
begin
  ...
  if (not regexp_like(v_obj, '^[A-Za-z0-9_$.]+$')) then
    return 'Unsafe INPUT_TABLE/INPUT_REF name (allowed: A-Za-z0-9_$.): ' || v_obj;
  end if;

  insert into FROSTBYTE_RUNS (
  ...
  v_sql := 'create or replace temporary table FROSTBYTE_BASE as
            select
  ...
  select count(*) into :v_rows_scanned from FROSTBYTE_BASE;
  ...
  create or replace temporary table FROSTBYTE_SERIES_ALL as
  select
  ...
  v_sql := 'create or replace temporary table FROSTBYTE_SERIES_KEEP as
            select *
            from FROSTBYTE_SERIES_ALL
            order by ROW_COUNT desc
            limit ' || to_varchar(v_max_series);
  execute immediate :v_sql;

  select count(*) into :v_series_count from FROSTBYTE_SERIES_KEEP;
  ...
  delete from FROSTBYTE_DQ_RUN where RUN_ID=:v_run_id;
  insert into FROSTBYTE_DQ_RUN
  with kept as (
    select b.*
    from FROSTBYTE_BASE b
    join FROSTBYTE_SERIES_KEEP k
  ...
  delete from FROSTBYTE_DQ_SERIES where RUN_ID=:v_run_id;
  insert into FROSTBYTE_DQ_SERIES
  with kept as (
    select b.*
    from FROSTBYTE_BASE b
    join FROSTBYTE_SERIES_KEEP k
  ...
  delete from FROSTBYTE_SERIES_WEEKLY where RUN_ID=:v_run_id;
  insert into FROSTBYTE_SERIES_WEEKLY
  with kept as (
    select b.*
    from FROSTBYTE_BASE b
    join FROSTBYTE_SERIES_KEEP k
  ...
  delete from FROSTBYTE_ANOMALIES_WEEKLY where RUN_ID=:v_run_id;

  v_sql := 'insert into FROSTBYTE_ANOMALIES_WEEKLY
            with w as (
              select *
              from FROSTBYTE_SERIES_WEEKLY
  ...
  delete from FROSTBYTE_FORECAST_4W where RUN_ID=:v_run_id;
  insert into FROSTBYTE_FORECAST_4W
  with w as (
    select *
    from FROSTBYTE_SERIES_WEEKLY
    where RUN_ID=:v_run_id
  ...
  delete from FROSTBYTE_SERIES_FEATURES where RUN_ID=:v_run_id;
  insert into FROSTBYTE_SERIES_FEATURES
  with w as (
    select *
    from FROSTBYTE_SERIES_WEEKLY
    where RUN_ID=:v_run_id
  ...
  anr as (
    ...
    from FROSTBYTE_ANOMALIES_WEEKLY
    where RUN_ID=:v_run_id
    group by 1,2,3,4,5,6
  ),
  miss as (
  ...
  delete from FROSTBYTE_ACTIONS where RUN_ID=:v_run_id;
  insert into FROSTBYTE_ACTIONS
  with dq as (
    select *
    from FROSTBYTE_DQ_RUN
    where RUN_ID=:v_run_id
  ),
  top_anom as (
    select *
  ...
  if (:v_enable_cluster) then
    call RUN_FROSTBYTE_CLUSTER(:v_run_id);
  end if;

  delete from FROSTBYTE_WEEKLY_BRIEF where RUN_ID=:v_run_id;
  if (:v_enable_cortex) then
    v_sql := '
      insert into FROSTBYTE_WEEKLY_BRIEF
      with dq as (
        select *
        from FROSTBYTE_DQ_RUN
        where RUN_ID = '''||replace(v_run_id,'''','''''')||'''
    ...
    v_sql := '
      insert into FROSTBYTE_AI_AUDIT
      select
        RUN_ID,
        CONFIG_NAME,
        COMPUTED_AT,
        ...
        BRIEF as RESPONSE
      from FROSTBYTE_WEEKLY_BRIEF
      where RUN_ID = '''||replace(v_run_id,'''','''''')||'''';
    execute immediate :v_sql;
  end if;

  update FROSTBYTE_RUNS
  set STATUS='SUCCESS',
      STATUS_MESSAGE=case when :v_series_count < (select count(*) from FROSTBYTE_SERIES_ALL) then 'CAPPED_SERIES: used top '||:v_series_count else null end,
      FINISHED_AT=current_timestamp(),
      WINDOW_START=:v_start,
      WINDOW_END=:v_end,
      LATEST_WEEK_START=:v_latest_week,
...
call RUN_FROSTBYTE('demo');

select *
from FROSTBYTE_RUNS
where CONFIG_NAME='demo'
...
from FROSTBYTE_ANOMALIES_WEEKLY
where CONFIG_NAME='demo'
order by COMPUTED_AT desc, abs(Z_SCORE) desc
limit 25;

select *
from FROSTBYTE_FORECAST_4W
where CONFIG_NAME='demo'
order by COMPUTED_AT desc, FORECAST_WEEK_START asc
limit 25;
...
-- 1) Run + config context (join so consumers can stay on secure views)
create or replace secure view VW_RUN_CONTEXT as
select
  r.RUN_ID,
  r.CONFIG_NAME,
  r.STATUS,
...
from FROSTBYTE_RUNS r
join FROSTBYTE_CONFIG c
  on c.CONFIG_NAME = r.CONFIG_NAME;

-- 2) Direct passthrough views (stable contracts)
create or replace secure view VW_DQ_RUN as
select * from FROSTBYTE_DQ_RUN;

create or replace secure view VW_DQ_SERIES as
select * from FROSTBYTE_DQ_SERIES;

create or replace secure view VW_SERIES_TIMELINE as
select * from FROSTBYTE_SERIES_WEEKLY;

create or replace secure view VW_FORECAST as
select * from FROSTBYTE_FORECAST_4W;

create or replace secure view VW_FEATURES as
select * from FROSTBYTE_SERIES_FEATURES;

create or replace secure view VW_ACTIONS as
select * from FROSTBYTE_ACTIONS;

create or replace secure view VW_WEEKLY_BRIEF as
select * from FROSTBYTE_WEEKLY_BRIEF;

create or replace secure view VW_AI_AUDIT as
select * from FROSTBYTE_AI_AUDIT;

-- 3) Clusters view (your base cluster table lacks CONFIG_NAME, so add it)
create or replace secure view VW_CLUSTERS as
select
  c.RUN_ID,
  r.CONFIG_NAME,
  c.COMPUTED_AT,
  ...
  c.DIM3,
  c.CLUSTER_ID
from FROSTBYTE_SERIES_CLUSTERS c
join FROSTBYTE_RUNS r
  on r.RUN_ID = c.RUN_ID;

-- 4) Latest-week anomalies view (what the UI expects for “inbox” patterns)
create or replace secure view VW_TOP_ANOMALIES as
with lw as (
...
select
  a.*
from FROSTBYTE_ANOMALIES_WEEKLY a
join lw
  on lw.RUN_ID = a.RUN_ID
 and lw.LATEST_WEEK_START = a.WEEK_START;

-- 5) Movers view (WoW change at latest week vs previous week, per run)
create or replace secure view VW_TOP_MOVERS as
with lw as (
  select
    RUN_ID,
    CONFIG_NAME,
...
-- 6) Dashboard KPIs view (single row per run_id with key counts)
create or replace secure view VW_DASHBOARD_KPIS as
with an as (
...
-- 7) Forecast joined view (forecast + last actual + health/features for confidence UI)
create or replace secure view VW_FORECAST_JOINED as
with lw as (
  select RUN_ID, LATEST_WEEK_START
  from FROSTBYTE_RUNS
)
select
  ...
  feat.ANOMALY_RATE,
  feat.SLOPE
from FROSTBYTE_FORECAST_4W f
join lw
  on lw.RUN_ID = f.RUN_ID
left join FROSTBYTE_SERIES_WEEKLY a
...
left join FROSTBYTE_DQ_SERIES dq
...
left join FROSTBYTE_SERIES_FEATURES feat
